...
function openPage(evt, pageName) {
    ...
    modalCntnts[0].className += " infocus";
    ...
    leftArrow.addEventListener("click", () => {
        i--;
        if (i < 0) { i = 0 }
        else if (i >= utility.imgData.length) { i = utility.imgData.length - 1 }
        if (i < utility.imgData.length && i >= 0) {
            modalCarousel[0].innerHTML = modalCarousel[0].innerHTML.replace(/ infocus/g, "");
            // for (let j=i;j>=0;j--){
            //     modalCntnts[i].style.transform= "scale("+j/i+")";
            // }
            modalCntnts[i].classList.add("infocus");
            modalDesc.innerHTML = utility.imgData[i].description;
        }
        activateKey(i);
    });
    ...
    rightArrow.addEventListener("click", () => {
        i++;
        if (i < 0) { i = 0 }
        else if (i >= utility.imgData.length) {
            i = utility.imgData.length - 1
        }
        if (i < utility.imgData.length && i >= 0) {
            modalCarousel[0].innerHTML = modalCarousel[0].innerHTML.replace(/ infocus/g, "");
            // for (let j=i;j<utility.imgData.length;j++){
            //     modalCntnts[i].style.transform= "scale("+i/j+")";
            // }
            modalCntnts[i].classList.add("infocus");
            modalDesc.innerHTML = utility.imgData[i].description;
        }
